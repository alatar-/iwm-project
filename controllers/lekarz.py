...
def wizyty():
    # |
    grid = SQLFORM.grid((db.visit.id_doctor == auth.user_id) & (db.visit.visit_day >= request.now.isoformat()),
...
def my_form_processing(form):
    ...
    if form.vars.office_begin >= form.vars.office_end:
        ...
        for row in rows:
            if form.vars.office_begin  >= row.office_end or form.vars.office_end <= row.office_begin:
                pass
            else:
...
def plan():
    ...
    grid = SQLFORM.grid(db.office_hours.id_doctor == auth.user_id,
                        user_signature=False,
                        details=False,
                        editable=False,
